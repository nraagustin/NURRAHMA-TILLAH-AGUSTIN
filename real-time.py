...
CATEGORIES = [ "DUDUK", "JATUH", "BERDIRI"]
FRAME_COUNT = 19
FRAME_SIZE = (64, 64)
...
class RealTimeActionClassifier:
   ...
   def draw_interface(self, frame):
       ...
       cv2.putText(frame, f"Frame Time: {self.frame_time*1000:.0f}ms", (10, 120), 
                  cv2.FONT_HERSHEY_SIMPLEX, 0.4, (255, 255, 255), 1)
       
       buffer_status = f"Buffer: {len(self.processed_buffer)}/{FRAME_COUNT}"
       cv2.putText(frame, buffer_status, (width-150, 30), 
                  cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)
       ...
       cv2.putText(frame, status_text, (width-150, 60), 
       ...
       if self.fall_detected:
           ...
           cv2.putText(frame, "FALL DETECTED!", (width//2-80, height//2), 
       ...
       if len(self.prediction_history) > 1:
           ...
           cv2.rectangle(frame, (chart_x, chart_y), (chart_x + chart_w, chart_y + chart_h), 
                        (50, 50, 50), -1)
           cv2.putText(frame, "History", (chart_x + 5, chart_y + 15), 
                      cv2.FONT_HERSHEY_SIMPLEX, 0.3, (255, 255, 255), 1)
           
           history_list = list(self.prediction_history)[-5:]
           for i, pred in enumerate(history_list):
               color = COLOR_MAP.get(pred['label'], (255, 255, 255))
               bar_height = int(pred['confidence'] * 20)
               bar_x = chart_x + 5 + i * 15
               bar_y = chart_y + chart_h - 5
               
               cv2.rectangle(frame, (bar_x, bar_y - bar_height), (bar_x + 10, bar_y), 
                            color, -1)
...
def main():
   ...
   classifier.frame_time = 1.0 / TARGET_FPS if TARGET_FPS > 0 else 0.5
   
   classifier.run()

if __name__ == "__main__":
   MODEL_PATH = "/home/brinyuhu/Dokumen/TILA/tila-tf/3model9.h5"
   TARGET_FPS = 2 
